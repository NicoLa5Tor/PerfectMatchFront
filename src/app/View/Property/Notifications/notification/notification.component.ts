...
console.log(this.endPoint+"chatHub");
  ...
  this.hubConnection.on("ReceiveMessage", (user: Notification, message: string) => {
    console.log(user);
    this._apiNotification.getNotifications(1).subscribe(x=>{this.notifications=x;});
  });
  
  this._apiNotification.getNotifications(1).subscribe(x=>{this.notifications=x;console.log(x)});
}
...
public removeNotification(id:number){
  this._apiNotification.removeNotification(id).subscribe(x=>{console.log(x);
    this._apiNotification.getNotifications(1).subscribe(x=>{this.notifications=x;console.log(x)});})
}
